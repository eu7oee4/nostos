...
_FALLBACK = (
    ...
    "你有长期记忆（markdown 文件），分两种：事件用 memory_write_item"
    "（人、地方、工作、计划，短 id 如 name / hometown）；感受用 memory_write_feel"
    "（紧张、怕、烦、在意，带 intensity：low / mid / high）。一句话里常常两样都有，分两次调。"
    "需要核对细节时用 memory_read / memory_list。"
    "递到你眼前的记忆每条带记下的日期，那是当时的事，不一定是现在的情况。\n"
